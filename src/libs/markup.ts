...
class Markup {
  ...
  private updateCourse = async () => {
    const getLastFetchingMinutes = this._TimeFormatting.minutes(
      this._LastFetching
    );
    this._User = await database("GET_USER", { id: this._Id });

    if (getLastFetchingMinutes >= 10 || this._LastFetching === 1) {
      let fetchingCourses = await fetch(
  ...
  getBusinesses = async () => {
    ...
    const newArr = businesses.map((item) => {
      ...
      let earnings = Math.floor(
        ((cost / 100) * config.business.payback.earnings) / 24
      );
  ...
  getJobs = async () => {
    ...
    const newArr = jobs.map((item) => {
      ...
      return {
        ...
        maxEarningsString: new Intl.NumberFormat("ru").format(
          Math.floor(earnings * config.job.factory)
        ),
